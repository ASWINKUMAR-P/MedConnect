...
export default function Tags() {
    ...
    const fetchTags = async () => {
        await fetch("http://localhost:8000/api/getAllTags", {
            method: "GET",
            headers: {
                                ...
                                {tags.length > 0 && tags.map(tag => (
                                    <div class="col">
                                        <div class="card h-100">
                                            ...
                                            <div class="card-body">
                                                <NavLink className="card-title p-1" to={{ pathname: `/questionOntags/${tag.name}` }} Style="text-decoration:none;color:hsl(205,47%,42%); background-color: hsl(205,46%,92%); border-radius:5px; display:inline;">{tag.name}</NavLink>
                                            </div>
                                        </div>
                                    </div>
                                ))}
